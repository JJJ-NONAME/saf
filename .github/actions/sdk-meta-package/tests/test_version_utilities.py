...
def test_extract_package_name() -> None:
    assert (
        version_utilities.extract_package_name("ansys_saf_glow_engine[dash]>=2.2.0")
        == "ansys_saf_glow_engine"
    )
    assert (
        version_utilities.extract_package_name("ansys-saf-desktop-installer")
...
def test_extract_lower_bound() -> None:
    assert (
        version_utilities.extract_lower_bound("ansys-saf-glow-engine>=2.2.0,<2.3.0")
        == "2.2.0"
    )
    assert (
        version_utilities.extract_lower_bound("ansys-saf-glow-engine==2.2.0") == "2.2.0"
    )
...
def test_determine_update_type() -> None:
    ...
    assert (
        version_utilities.determine_update_type(
            version_utilities.UpdateType.AUTO, current, {"package": "1.0.0"}
        )
        is version_utilities.UpdateType.NO_UPDATE
    )
    assert (
        version_utilities.determine_update_type(
            version_utilities.UpdateType.AUTO, current, {"package": "1.0.1"}
        )
        is version_utilities.UpdateType.PATCH
    )
    assert (
        version_utilities.determine_update_type(
            version_utilities.UpdateType.AUTO, current, {"package": "1.1.0"}
        )
        is version_utilities.UpdateType.MINOR
    )
    assert (
        version_utilities.determine_update_type(
            version_utilities.UpdateType.AUTO, current, {"package": "2.0.0"}
        )
...
def test_bump_version() -> None:
    assert (
        version_utilities.bump_version("1.2.3", version_utilities.UpdateType.PATCH)
        == "1.2.4"
    )
    assert (
        version_utilities.bump_version("1.2.3", version_utilities.UpdateType.MINOR)
        == "1.3.0"
    )
    assert (
        version_utilities.bump_version("1.2.3", version_utilities.UpdateType.MAJOR)
        == "2.0.0"
    )
    assert (
        version_utilities.bump_version("1.2.3", version_utilities.UpdateType.NO_UPDATE)
        == "1.2.3"
    )
...
def test_meta_package_release_exists(
    releases: dict[str, list[object]], expected: bool
) -> None:
    response = Mock()
...
def test_get_latest_stable_versions() -> None:
    with patch.object(
        version_utilities, "get_latest_stable_version", side_effect=["1.0.0", "2.0.0"]
    ) as get:
...
def test_get_latest_stable_versions_uses_selected_versions() -> None:
    with (
        ...
        assert version_utilities.get_latest_stable_versions(
            ["ansys-bdm-api", "ansys-saf-desktop-orchestrator"]
        ) == {"ansys-bdm-api": "0.6.1", "ansys-saf-desktop-orchestrator": "1.0.0"}

    get.assert_called_once_with("ansys-saf-desktop-orchestrator")


def test_build_updated_requirement_uses_selected_version_for_optional_dependency() -> (
    None
):
    with patch.dict(
        os.environ,
        ...
        versions = version_utilities.get_latest_stable_versions(
            ["ansys-saf-desktop-installer"]
        )

    assert (
        version_utilities.build_updated_requirement(
            "ansys-saf-desktop-installer[all]>=1.0.0,<1.1.0", versions
        )
...
def test_get_meta_package_version_when_not_published() -> None:
    ...
    response.raise_for_status.side_effect = version_utilities.requests.HTTPError(
        response=response
    )
    with patch.object(version_utilities.requests, "get", return_value=response):
        assert (
            version_utilities.get_meta_package_version(
                version_utilities.UpdateType.MINOR
            )
...
def test_get_dependency_pinning_rejects_invalid_value() -> None:
    with patch.dict(
        os.environ, {version_utilities.DEPENDENCY_PINNING_ENV_VAR: "invalid"}
    ):
        with pytest.raises(
            ValueError,
...
def test_build_updated_requirement() -> None:
    ...
    assert (
        version_utilities.build_updated_requirement(
            "ansys-saf-glow-engine[dash]>=2.2.0,<2.3.0", versions
        )
        == "ansys-saf-glow-engine[dash]==2.4.1"
    )
    assert (
        version_utilities.build_updated_requirement("requests>=2.0.0", versions)
        == "requests>=2.0.0"
    )

    with patch.dict(
        os.environ, {version_utilities.DEPENDENCY_PINNING_ENV_VAR: "range"}
    ):
        assert (
            version_utilities.build_updated_requirement(
                "ansys-saf-glow-engine[dash]>=2.2.0,<2.3.0", versions
...
def test_validate_user_selected_update_type_allows_non_release_branch(
    ...
    with (
        ...
        patch.object(
            version_utilities, "bump_version", return_value="1.2.3"
        ) as bump_version,
        patch.object(
            version_utilities, "release_branch_exists", return_value=False
        ) as release_branch_exists,
        patch.object(
            version_utilities, "meta_package_release_exists", return_value=False
        ) as meta_package_release_exists,
    ):
        assert version_utilities.validate_user_selected_update_type(
            branch
        ) is version_utilities.UpdateType(update_type)

    bump_version.assert_called_once_with(
        "1.2.3", version_utilities.UpdateType(update_type)
    )
    release_branch_exists.assert_called_once_with("1.2.3")
    meta_package_release_exists.assert_called_once_with("1.2.3")


def test_validate_user_selected_update_type_allows_auto_without_existence_check_on_non_release_branch() -> (
    None
):
    branch = Mock(is_release_branch=False, version="1.2.3")
    with (
        ...
        patch.object(
            version_utilities, "release_branch_exists"
        ) as release_branch_exists,
        patch.object(
            version_utilities, "meta_package_release_exists"
        ) as meta_package_release_exists,
    ):
        assert (
            version_utilities.validate_user_selected_update_type(branch)
            is version_utilities.UpdateType.AUTO
        )
...
def test_validate_user_selected_update_type_checks_auto_as_patch_on_release_branch() -> (
    None
):
...
def test_validate_user_selected_update_type_rejects_non_patch_release_update_types(
    ...
    branch = Mock(is_release_branch=True, version="1.2.3")
    with patch.dict(os.environ, {"USER_SELECTED_UPDATE_TYPE": update_type}):
        with pytest.raises(ValueError, match=r"Valid values are \['auto', 'patch'\]"):
            version_utilities.validate_user_selected_update_type(branch)


@pytest.mark.parametrize(
    ("branch_exists", "pypi_release_exists"), [(True, False), (False, True)]
)
def test_validate_user_selected_update_type_rejects_existing_next_version(
    branch_exists: bool, pypi_release_exists: bool
) -> None:
    branch = Mock(is_release_branch=True, version="1.2.3")
    with (
        patch.dict(os.environ, {"USER_SELECTED_UPDATE_TYPE": "patch"}),
        patch.object(
            version_utilities, "release_branch_exists", return_value=branch_exists
        ),
        patch.object(
            version_utilities,
