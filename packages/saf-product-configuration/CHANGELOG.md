# Changelog

This project uses [towncrier](https://towncrier.readthedocs.io/) to generate changelogs.

Refer to the [raw release notes](doc/source/changelog.rst) for more information.

[Published release notes](http://saf-product-configuration.docs.solutions.ansys.com/version/stable/changelog.html) can be found in the online documentation.
