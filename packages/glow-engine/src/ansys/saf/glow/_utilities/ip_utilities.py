...
def wait_for_response(url: str, tries: int = 7, interval: float = 5):
    ...
    while not response:
        ...
        logger.info(f"Waiting {interval}s. (To give " + url + " time to respond)")
        time.sleep(interval)
        response = try_response(url)
        i = i - 1


def try_response(url: str, timeout: int = 5) -> bool:
    ...
    try:
        ...
        with httpx2.Client(timeout=timeout, transport=InjectTraceTransport()) as http_client:
