...
pytestmark = pytest.mark.parametrize("solution_type", [SolutionAssetsWithAutoKey], indirect=True)


def test_always_decrypted_asset(function_project: ProjectFixture[SolutionAssetsWithAutoKey]):
...
def test_decrypted_for_debug_asset(function_project: ProjectFixture[SolutionAssetsWithAutoKey]):
...
def test_asset_dir(function_project: ProjectFixture[SolutionAssetsWithAutoKey]):
    ...
    assert content.rstrip() == "always decrypted"
