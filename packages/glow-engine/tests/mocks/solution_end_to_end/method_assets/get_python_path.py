...
class GetPythonPath(HpsExecution):

    def execute(self) -> Dict[str, Union[int, float, str, bool]]:
        python_product = self.context.products[0]
        return {"python_name": python_product.name, "python_version": python_product.version.replace('.', '_')}
