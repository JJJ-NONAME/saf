...
class MyStep(StepModel):
    ...
    @create_instance("my_step.x", EmptyInstanceManager, identifier="b")
...
class MethodCreateInstanceTwiceMixedStepInstanceReference(Solution):
    display_name: str = (
        "Method: @create_instance twice referring to the same instanceusing references with and without dot notation"
    )
    steps: Steps
