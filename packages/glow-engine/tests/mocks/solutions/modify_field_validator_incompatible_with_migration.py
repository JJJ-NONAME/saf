...
class MigrateToVersion2(MigrationTransformation):
    def migrate(self, ctx: MigrationContext):
        ctx.steps["first_step"]["unexpected_int"] = 100


class MigrateFieldDefaultValue(MigrationTransformation):
    def migrate(self, ctx: MigrationContext) -> None:
        ctx.steps["first_step"]["unused_value"] = 2
...
class FirstStep(StepModel):
    ...
    str_int: str = "1"
    unexpected_int: int = 100

    @field_validator("unused_value")  # type: ignore
    @classmethod
    def must_be_greater_than_1(cls, v: float) -> float:
        if v <= 1.0:
            raise ValueError(f"{v} must be greater than 1")
        return v
...
class OriginalSolution(Solution):
    ...
    migrations: list[Migration] = [
        ...
        Migration(version=2, migration_transformation=MigrateFieldDefaultValue()),
    ]
