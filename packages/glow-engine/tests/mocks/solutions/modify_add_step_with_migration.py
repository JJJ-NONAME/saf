...
class MigrateAddStep(MigrationTransformation):
    def migrate(self, ctx: MigrationContext):
        ctx.steps["third_step"] = {"x": 88, "state": {"x": FieldState.OUTOFDATE}}
...
class ThirdStep(StepModel):
    x: int = 88

    @transaction(self=StepSpec(upload=["x"]))
    def generate_random_int(self) -> None:
...
class OriginalSolution(Solution):
    ...
    migrations: list[Migration] = [
        ...
        Migration(version=2, migration_transformation=MigrateAddStep()),
    ]
