...
class SetEntityHandleToNoEntity(MigrationTransformation):
    def migrate(self, ctx: MigrationContext) -> None:
        ctx.steps["first_step"]["new_entity_handle"] = NO_ENTITY.model_dump()
        _remove_old_fields(ctx)


class FirstStep(StepModel):
    x: int = 88
    new_entity_handle: EntityHandle = NO_ENTITY
...
class OriginalSolution(Solution):
    ...
    steps: Steps
    migrations: list[Migration] = [Migration(version=1, migration_transformation=SetEntityHandleToNoEntity())]
