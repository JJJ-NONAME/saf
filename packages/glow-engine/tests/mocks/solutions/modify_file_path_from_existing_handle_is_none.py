...
class CheckEntityHandle(MigrationTransformation):
    def migrate(self, ctx: MigrationContext) -> None:
        assert ctx.get_path_from_entity_handle(ctx.steps["first_step"]["always_no_entity_handle"]) is None
...
class OriginalSolution(Solution):
    ...
    steps: Steps
    migrations: list[Migration] = [Migration(version=1, migration_transformation=CheckEntityHandle())]
