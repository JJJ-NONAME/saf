...
class MigrateWithError(MigrationTransformation):
    def migrate(self, ctx: MigrationContext):
        new_file = ctx.project_directory / "new_file.txt"
        if new_file.is_file():
            new_file.unlink()
...
class OriginalSolution(Solution):
    ...
    migrations: list[Migration] = [
        ...
        Migration(version=2, migration_transformation=MigrateWithError()),
    ]
