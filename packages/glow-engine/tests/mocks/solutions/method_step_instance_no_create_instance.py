...
class MyStep(StepModel):
    ...
    # the instance x is not created for other_step
    @transaction()
    @instance("other_step.x")
...
class MethodStepInstanceSaveToNotState(Solution):
    display_name: str = (
        "Method: @instance when referring to other step instance - there is no matching @create_instance"
    )
    steps: Steps
