...
from tests.mocks.solutions.solution_with_minimal_dash_ui.solution.definition import MySolution
...
def crash_pythonw() -> None:
    """This is simulating code that crashes pythonw.exe if subprocess stdout/stderr are not redirected to /dev/null."""
    for _ in range(1000):
        # (using subprocess.PIPE, the subprocess can block if the pipe buffer fills up and nobody reads from it)
        print("Large output line with lots of data...")
