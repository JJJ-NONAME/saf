...
from ansys.saf.desktop.orchestrator._config.schema import (
    DEPRECATED_INSTANCE_CONFIGS_DIR_NAME,
    PRODUCT_INSTANCE_CONFIGS_DIR_NAME,
)
...
def find_product_instance_configs_dir(solution_module: ModuleType) -> Path | None:
    ...
    for parent in Path(solution_module.__file__).parents:
        ...
        deprecated_instance_configs_dir = parent / DEPRECATED_INSTANCE_CONFIGS_DIR_NAME
        if deprecated_instance_configs_dir.is_dir():
            logger.warning(
                f"Found directory with product instance configurations: {deprecated_instance_configs_dir}"
                " which is deprecated. Use 'product_instance_configs' instead.",
            )
            return deprecated_instance_configs_dir
