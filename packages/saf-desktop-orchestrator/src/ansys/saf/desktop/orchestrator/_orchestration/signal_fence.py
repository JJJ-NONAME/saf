...
@contextmanager
def signal_fence(  # noqa: C901
    signum: signal.Signals,
    *,
    on_deferred_signal: Callable[[int, FrameType | None], None] | None = None,
) -> Iterator[None]:
    """A `signal_fence` creates an uninterruptible "fence" around a
    block of code. The fence defers a specific signal received inside
    of the fence until the fence is destroyed, at which point the
    original signal handler is called with the deferred signal.
    Multiple deferred signals will result in a single call to the
    original handler. An optional callback `on_deferred_signal` may be
    specified which will be called each time a signal is handled while
    the fence is active, and can be used to print a message or record
    the signal.
    ...
    """
    handled: tuple[int, FrameType | None] | None = None
    ...
    if isinstance(original_handler, int) and not isinstance(original_handler, signal.Handlers):
