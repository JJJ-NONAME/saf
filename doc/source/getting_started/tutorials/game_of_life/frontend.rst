...
.. topic:: Objective
  ...
  **In this module, you'll cover the following topics:**
  ...
  - :material-outlined:`play_circle;1.25em;saf-objective-icon` Run the finished solution with
    ``saf run`` and watch the Game of Life evolve.
...
Most of it is plain Dash and Mantine markup. The two SAF-specific lines are right at the end:
...
.. tip::
    ...
    small when only one property changes.
...
- ``prevent_initial_call=True`` prevents the callback from firing when the page first loads —
  otherwise it would start a simulation nobody asked for.
...
.. practice::
    ...
    - Drag the grid-size slider — same thing.
    ...
    If something goes wrong, watch the terminal output: SAF prints tracebacks from failing
    transactions there. Common issues:
...
.. important::
    ...
    - The frontend interacts with the backend **through the ``DashClient``** — no manual HTTP
      calls. Assign to a field and it is persisted. Call a transaction method and it runs.
