...
.. _saf-ex-mechanical-product-instance-summary:
...
This example demonstrates how to create a product instance manager for Mechanical products using SAF. The product instance manager enables management of the Mechanical product instance lifecycle, including starting, stopping, and accessing the product's API.

.. list-table:: **Mechanical Product Instance Manager**
   :widths: 20 80
   :header-rows: 1
   ...
   * - Step model
     - * Long-running transaction to initialize the product instance.
       ...
       * Long-running transaction to call the Mechanical API to run a custom script.
       * Transaction to call the Mechanical API to download the output file to the current working directory.
   * - User interface
...
To create a product instance manager for Mechanical products and interact with it, walk through the following sequence of tabs.

.. tab-set::

  .. tab-item:: 1️⃣Backend
    :name: saf-ex-mechanical-product-instance-backend-tab
  ...
  .. tab-item:: 2️⃣Frontend
    :name: saf-ex-mechanical-product-instance-frontend-tab
