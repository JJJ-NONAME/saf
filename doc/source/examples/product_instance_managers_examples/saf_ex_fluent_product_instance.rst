...
.. _saf-ex-fluent-product-instance-summary:
...
.. list-table:: **Fluent Product Instance Manager**
   :widths: 20 80
   :header-rows: 1
   ...
   * - Step model
     - * Long-running transaction to initialize the product instance.
       ...
       * Long-running transaction to call the Fluent API to import a mesh.
       * Long-running transaction to call the Fluent API to run a simulation.
   * - User interface
     - * Button to initialize the product instance.
       * Button to shut down the product instance.
       * Button to import a mesh.
...
To create a product instance manager for Fluent products and interact with it, walk through the following sequence of tabs.

.. tab-set::

  .. tab-item:: 1️⃣Backend
    :name: saf-ex-fluent-product-instance-backend-tab
  ...
  .. tab-item:: 2️⃣Frontend
    :name: saf-ex-fluent-product-instance-frontend-tab
