...
.. tab-set::
  ...
  .. tab-item:: 2️⃣Solution components
   ...
   To continue, click the **3️⃣ Ansys viewers** tab.


  .. tab-item:: 3️⃣ Ansys viewers
   :name: saf-ex-dash-components-tab-3

   The **Ansys Dash Viewers** library (``ansys-dash-viewers``) provides ADR 3D Viewer web components as Dash-ready components, enabling seamless integration of 3D visualization capabilities into Dash applications.

   The ``Ansys3DViewer`` component is capable of rendering 3D geometry files generated by Ansys products, including formats like ``.avz``, ``.dsco``, and ``.scdoc``.


   .. dropdown:: Getting started
    :open:

    #. To access the components from **Ansys Dash Viewers** library, install the package with Poetry:

       .. code-block:: bash

          poetry add ansys-dash-viewers --source solutions-private-pypi

    #. To use the components, you must be able to serve the **ADR 3D Viewer** static files. To ensure that the static assets are served correctly, add the following code to the ``app.py`` file of the solution frontend:

       .. code-block:: python
         :caption: <solution_name>/ui/app.py

          import ansys_dash_viewers

          ansys_dash_viewers.serve_viewer_assets(app)

    #. The ``ansys_dash_viewers`` package gives you access to the ``Ansys3DViewer`` component. Import the component as follows:

       .. code-block:: python

         from ansys_dash_viewers import Ansys3DViewer

    Review the viewer code and output examples that follow.


   .. dropdown:: Viewer 1 - AVZ File
     :open:

     The following code uses the ``Ansys3DViewer`` component to render a ``.avz`` file:
     ...
     .. code-block:: python
      ...
      Ansys3DViewer(
          id="avz-viewer",
          src="/assets/geometries/viewer_test.avz",
          aspectRatio="1.0",
          active=True,
      ),

     .. image:: /_static/images/viewer1.gif
      :align: center
      :width: 65%
   ...
   .. dropdown:: Viewer 2 - SCDOC File
    :open:

    The following code uses the ``Ansys3DViewer`` component to render a ``.scdoc`` file:

    .. code-block:: python

     Ansys3DViewer(
         id="scdoc-viewer",
         src="/assets/geometries/viewer_test.scdoc",
         aspectRatio="1.0",
         active=True,
     ),

    .. image:: /_static/images/viewer2.gif
     :align: center
     :width: 65%



   .. dropdown:: Viewer 3 - DSCO File
     :open:

     The following code uses the ``Ansys3DViewer`` component to render a ``.dsco`` file.

     .. code-block:: python

      Ansys3DViewer(
          id="scdoc-viewer",
          src="/assets/geometries/Piston_Crank.dsco",
          aspectRatio="1.0",
          active=False,
      ),

     .. image:: /_static/images/viewer3.gif
      :align: center
      :width: 65%
